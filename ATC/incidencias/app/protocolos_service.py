...
class ProtocolosService:
    ...
    def _contactos_para_informe(self, informe: ProtocoloInforme) -> list[dict[str, str]]:
        ...
        if cliente_match:
            ...
            add("Email facturas", cliente_match.email_facturas or "", "Cliente")
        ...
        if sucursal_match:
            add("Email facturas sucursal", sucursal_match.email_facturas or "", "Sucursal")
    ...
    def _enviar_mail_protocolo(self, destinos: list[str], asunto: str, cuerpo: str, html: str) -> None:
    ...
    def enviar_informe_semanal(self, informe_id: int, payload: dict[str, Any]) -> dict[str, object]:
        ...
        cuerpo = (
            f"Estimado cliente,\n\n"
            f"Compartimos el informe semanal de protocolos de {informe.sucursal}.\n"
            f"Periodo: {periodo or 'No informado'}\n"
            f"PDF: {informe.pdf_url}\n\n"
            f"Saludos,\nATC"
        )
        html = f"""
        <div style="font-family:Arial,sans-serif;color:#111827;line-height:1.55">
          <h2 style="margin:0 0 10px">Informe semanal de protocolos</h2>
          <p>Estimado cliente, compartimos el informe semanal de protocolos correspondiente a <b>{informe.sucursal}</b>.</p>
          <p><b>Cliente:</b> {informe.cliente}<br><b>Periodo:</b> {periodo or 'No informado'}</p>
          <p><a href="{informe.pdf_url}" target="_blank" rel="noopener"
             style="display:inline-block;padding:10px 14px;background:#0b1424;color:#fff;text-decoration:none;border-radius:8px;font-weight:700">
             Abrir PDF
          </a></p>
          <p style="color:#4b5563">Saludos,<br>ATC</p>
        </div>
        """
        self._enviar_mail_protocolo(destinos, asunto, cuerpo, html)
