...
from ATC.incidencias.app.database import SessionLocal, build_engine
...
DEPARTMENT_AREAS: dict[str, list[str]] = {
    ...
    "tecnicos": ["tecnicos"],
    "técnicos": ["tecnicos"],
    "operador": ["incidencias", "coordinacion", "protocolos"],
    "coordinacion": ["coordinacion"],
...
"""
    ...
    def _es_rut_login(self, valor: Any) -> bool:
        return bool(re.fullmatch(r"\d{7,8}-[0-9Kk]", str(valor or "").strip()))
    ...
    def _usuarios_login_por_area(self, area_code: str) -> list[str]:
        ...
        nombres = [u.name for u in usuarios if area_code in self._area_codes_usuario(u)]
        return sorted(nombres, key=lambda x: self._normalizar_nombre_login(x))

    def obtener_usuarios_login_detalle(self, destino: str = "tecnicos") -> list[dict[str, Any]]:
        if self._es_destino_auto(destino):
            detalle: list[dict[str, Any]] = []
            for user in self.db.scalars(select(User).where(User.is_active.is_(True)).order_by(User.name.asc())).all():
                codes = self._area_codes_usuario(user)
    ...
    def _redirect_selector_areas(self, app_url: str, token: str) -> str:
        ...
        if helpdesk:
            return f"{helpdesk}/sso/login?token={token}"
        return f"{app_url}/seleccionar-area?token={quote_plus(token)}"
    ...
    def contar_areas_para_token(self, token: str) -> int:
        ...
        seen: set[str] = set()
        for dept in re.split(r"[;,|]+", raw):
            for code in dept_to_areas.get(dept.strip().casefold(), []):
                seen.add(code)
        return len(seen)
    ...
    def obtener_resumen_equipos_tecnicos_hoy(self) -> dict[str, Any]:
        ...
        patentes_por_tecnico = {
            "nicolas bravo": "KVTG 28",
            "emmanuel correa": "RHPV 38",
            "marco lopez": "RJXX 46",
            "ricardo vergara": "RTXG 52",
            "javier salgado": "SRVP 17",
            "por confirmar": "SSZS 24",
            "*por confirmar*": "SSZS 24",
            "bryan ibaceta": "SSZW 51",
            "luis bustamante": "VXLG 86",
            "jesus gonzalez": "VXLG 93",
        }
        patentes_fijas = [
            "KVTG 28",
            "RHPV 38",
        ...
        def _estado_activo(value: str) -> bool:
            estado_norm = self._normalizar_texto(value)
            return not any(x in estado_norm for x in ["termin", "repetida", "anulada", "finalizado"])
        ...
        def _agregar_odt(
            *,
            ...
            if not tecnico_limpio and not acompanante_limpio:
                return
            if not _fecha_en_hoy(fecha_ref) and not (incluir_si_activa and _estado_activo(estado)):
                return
            ...
            if key not in equipos:
                ...
                equipos[key] = {
                    ...
                    "miembros": miembros,
                    "patente": _patente_tecnico(tecnico_limpio) or _patente_tecnico(acompanante_limpio),
        ...
        rows = (
            self.db.scalars(
                ...
                .order_by(Registro.fecha_derivacion_tecnico.desc().nullslast(), Registro.fecha_derivacion_area.desc().nullslast(), Registro.odt.asc())
            )
            .all()
        )
        ...
        for ods, st, adm in ods_rows:
            odt_key = self._normalizar_texto(ods.codigo)
            if odt_key and odt_key in odts_agregadas:
                continue
            tecnico = _clean(getattr(st, "tecnico_a_cargo", "") if st else "") or _clean(getattr(adm, "tecnico", "") if adm else "")
            acompanante = _clean(getattr(st, "acompanante", "") if st else "") or _clean(getattr(adm, "acompanante", "") if adm else "")
            fecha_ref = (
                (getattr(st, "updated_at", None) if st else None)
                or (getattr(st, "fecha_recepcion_solicitud_instalacion", None) if st else None)
                or (getattr(adm, "fecha_derivacion", None) if adm else None)
                or ods.created_at
            )
            antes = total_odt
            _agregar_odt(
                odt=ods.codigo,
                cliente=ods.nombre_sucursal or ods.razon_social,
                problema=ods.tipo_servicio,
                detalle=ods.observacion,
                direccion=ods.direccion_sucursal,
                estado=ods.estado,
                prioridad=None,
                fecha_ref=fecha_ref,
                tecnico=tecnico,
        ...
        for patente in patentes_fijas:
            ...
            if not equipo:
                equipo = {
                    "titulo": patente,
                    "miembros": [{"nombre": "Sin asignacion", "patente": patente}],
                    "patente": patente,
                    "tecnico": "",
                    "acompanante": "",
                    "odts": [],
...
<html lang="es">
  ...
  <body style="margin:0;padding:0;background:#f4f6f9;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Helvetica,Arial,sans-serif;color:#1f2937;">
        ...
        if accion == "aceptar":
            ...
            if estado_actual != "pagado":
                pago = RendicionPago(
                    ...
                    tecnico=str(rend.tecnico or "").strip(),
                    rut_tecnico=None,
    ...
    def obtener_viatico_especial(self, codigo: str = "", personalizados: bool = False) -> list[dict[str, Any]]:
        ...
        codigo_buscado = _normalizar_codigo(codigo)
        query = (
            select(Rendicion)
        ...
        caps_normalizados = {_normalizar_codigo(k): v for k, v in caps.items()}
        ...
        out: list[dict[str, Any]] = []
        for codigo, g in grupos.items():
            vmax = caps_normalizados.get(_normalizar_codigo(codigo), self.VIATICO_CAP_ESPECIAL_DEFECTO)
            real = g["real"]
            ...
            out.append({
                ...
                "personalizado": _normalizar_codigo(codigo) in caps_normalizados,
            })
        out.sort(key=lambda x: x["codigo"])
        return out
        ...
        return [
            {
                ...
                "suma": float(s or 0),
                "pagos": int(n or 0),
                "ultimoPago": _to_ddmmyyyy(u) if u else "",
            }
            for c, s, n, u in rows
        ]
