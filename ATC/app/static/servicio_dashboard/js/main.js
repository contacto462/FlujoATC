...
async function bootstrap() {
  try {
    ...
    DATA = sinPreventivas(registros);
    AVANCE = avance || [];
    IS_MOCK = mock;
    M.setConfig(config);
    M.marcarReincidencias(DATA);
...
function fillSelects() {
  ...
  fill('fCliente', M.uniq(DATA.map(r => (r.cliente || '').trim())).sort(), 'Cliente');
  fill('fEstado', M.uniq(DATA.map(r => (r.estado || '').trim())).sort(), 'Estado');
  fill('fTipo', M.uniq(DATA.map(r => (r.problema || '').trim())).sort(), 'Tipo');
  fill('fTecnico', M.uniq(DATA.flatMap(M.tecnicosDe)).sort(), 'Técnico');
...
function toggleTheme() {
  ...
  localStorage.setItem('atc_dash_theme', dark ? 'light' : 'dark');
  renderAll(); // re-renderiza gráficos con la nueva paleta
}
...
function renderAll() {
  ...
  UI.setText('filterMeta', data.length + ' de ' + DATA.length + ' ODT');
...
function renderReincidencias(data) {
  ...
  UI.setHTML('alertsGrid', alertas.length
    ...
    : '<div class="alerts-empty">✅ Ninguna sucursal registra 2 o más ODT en una misma semana dentro del período filtrado.</div>');
}
...
/* ── Tema inicial + arranque ─────────────────────────────────────── */
if (localStorage.getItem('atc_dash_theme') === 'dark') {
